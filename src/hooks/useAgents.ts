...
const DEFAULT_AGENT: CustomAgent = {
  id: '__default__',
  name: 'DocQA 助手',
  description: '智能文档问答专家，基于硅基流动 API',
...
export function useAgents() {
  const [agents, setAgents] = useState<CustomAgent[]>(() => {
    try {
      ...
      if (saved) {
        ...
        // 清理旧数据：移除 id 为 'default' 的 agent（已改为 '__default__'）
        const cleaned = parsed.filter((a: any) => a.id !== 'default');
        localStorage.setItem(STORAGE_KEY, JSON.stringify(cleaned));
        return [DEFAULT_AGENT, ...cleaned.map((a: any) => ({
          ...a,
  ...
  const saveAgents = useCallback((newAgents: CustomAgent[]) => {
    const toSave = newAgents.filter(a => a.id !== '__default__');
  ...
  const deleteAgent = useCallback((id: string) => {
    if (id === '__default__') return; // 不能删除默认 agent
    setAgents(prev => {
