...
// 数据库迁移：添加 provider 列（如果不存在）
try {
  const tableInfo = db.prepare("PRAGMA table_info(sessions)").all() as Array<{ name: string }>;
  const hasColumn = tableInfo.some(col => col.name === 'provider');
  if (!hasColumn) {
    db.exec("ALTER TABLE sessions ADD COLUMN provider TEXT DEFAULT 'codebuddy'");
    console.log("[DB] Added provider column to sessions table");
  }
} catch (e) {
  // 忽略错误（列可能已存在）
}
...
export function createSession(session: DbSession): DbSession {
  const stmt = db.prepare(`
    INSERT INTO sessions (id, title, model, sdk_session_id, provider, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
  stmt.run(session.id, session.title, session.model, session.sdk_session_id, session.provider || 'codebuddy', session.created_at, session.updated_at);
  return session;
}

// 更新会话
export function updateSession(id: string, updates: Partial<Pick<DbSession, 'title' | 'model' | 'sdk_session_id' | 'provider'>>): boolean {
  const fields: string[] = [];
  const values: any[] = [];
